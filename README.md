...
A visão `Decomp. RPS` substitui o waterfall principal por uma timeline de barras divergentes chamada `Decomposicao ao longo do lancamento`. Ela mostra, para cada semana fechada, a variação de receita contra a referência fixa `D0-D30` em dois drivers executivos: Sessões e RPS. A identidade usada é `Receita = Sessões x RPS`; o efeito de sessões é `(Sessões1 - Sessões0) * ((RPS0 + RPS1) / 2)` e o efeito de RPS é `(RPS1 - RPS0) * ((Sessões0 + Sessões1) / 2)`. A base D0-D30 é semanalizada para a mesma quantidade de dias da semana analisada, sem recalcular a régua a cada semana.
...
Na visão executiva, os gráficos de linha permanecem. A leitura principal usa dois gráficos empilhados e sincronizados no mesmo eixo D+: `PERFORMANCE` e `SUPORTE`. Performance mostra quanto do RPS do lançamento foi preservado, com `D0-D30 = 100`; Suporte mostra o esforço conhecido contra a mesma base, mas em escala Y própria.

A tela executiva evita legenda inferior e jargão na superfície. Cada linha recebe rótulo direto na ponta, como `96% Performance` e `62% Suporte`; a linha de referência recebe apenas `Base`. A performance usa zonas visuais discretas: `FORTE` acima de 100, `PRESERVADA` entre 90 e 100, e `ATENÇÃO` abaixo de 90. O suporte marca `100` como mesmo suporte do lançamento e `70` como suporte significativamente reduzido, sem tratar redução como boa automaticamente.

O topo da visão executiva usa uma única conclusão e somente três KPIs: `PERFORMANCE`, `SUPORTE` e `CONFIRMAÇÃO`. Os status permitidos são `INDEPENDENTE`, `INDEPENDÊNCIA EM FORMAÇÃO`, `DEPENDENTE`, `FORTE, MAS MUITO ESTIMULADO`, `PERDENDO FORÇA` e `DADOS INSUFICIENTES`. Valores executivos são arredondados; precisão completa e fórmulas ficam nos tooltips ou na visão analítica.

O gráfico opcional `O QUE SUSTENTA O RESULTADO?` aparece abaixo de Performance e Suporte. Ele mostra duas linhas, `Tráfego` e `Eficiência`, em eixo zero central: acima de zero contribui para receita; abaixo de zero reduz receita. O waterfall fica apenas em `Ver detalhes · 3 análises ↓`, como ferramenta analítica expandida inline. Ao abrir, a visão executiva permanece acima; ao fechar com `Ocultar detalhes ↑`, filtros, produto, período e ponto selecionado são preservados.

Autosustentação só é classificada quando existe RPS e esforço comparáveis depois da fase base. Se faltar esforço posterior, a linha de suporte fica com lacuna e a interface informa indisponibilidade; ausência de dado não vira zero. Desacoplamento de tráfego não é automaticamente autosustentação: a regra configurável inicial usa `AUTOSUSTAIN_RPS_INDEX_MIN = 90`, `AUTOSUSTAIN_EFFORT_INDEX_MAX = 70` e `AUTOSUSTAIN_MIN_CONSECUTIVE_WEEKS = 4`; esses parâmetros são hipóteses operacionais e não representam calibração estatística universal.
