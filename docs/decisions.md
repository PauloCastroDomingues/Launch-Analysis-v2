...
27. Atribuicao real de canal usa chave validada por email normalizado + data paga BRT + valor total arredondado. Como `mart_growth_us` fica em US e `mart_shared` em southamerica-east1, o dashboard consome a tabela espelho `mart_shared.canal_atribuicao_pedido_mirror`; `order_id`, `order_name` e `customer_sk` nao sao chave de juncao entre regioes.
